...
import { COMPLEX_PACKAGES } from "@/packages/ShaPackages"
...
export default function ComplexCaseBuilder({
  ...
  const currentNetValue =
    ...
    const fetchPackages = async () => {
        const allowedPackages = COMPLEX_PACKAGES;
        try {
        const pck = await getPackages();

        const filtered = pck.filter(pkg => allowedPackages.includes(pkg.code));
